WidgetMetadata = {
  id: "forward.danmu.pro.convert",
  title: "多源弹幕 (全量转码版)",
  version: "5.0.0",
  requiredVersion: "0.0.2",
  description: "支持3源切换，内置全量简繁体字库，支持分段加载",
  author: "Gemini Remix",
  ...
  globalParams: [
    ...
    // --- 源 2 ---
    {
      name: "s2_name", title: "📡 源2 名称 (选填)", type: "input"
    },
    {
      name: "s2_url", title: "📡 源2 地址 (选填)", type: "input"
    },
    // --- 源 3 ---
    {
      name: "s3_name", title: "📡 源3 名称 (选填)", type: "input"
    },
    {
      name: "s3_url", title: "📡 源3 地址 (选填)", type: "input"
    },
    // --- 功能配置 ---
    {
      name: "convertMode",
...
function getActiveServers(params) {
    ...
    if (params.s1_url) {
        ...
        list.push({ name: params.s1_name || "源1", url: cleanUrl(params.s1_url) });
    ...
    if (params.s2_url) {
        ...
        list.push({ name: params.s2_name || "源2", url: cleanUrl(params.s2_url) });
    ...
    if (params.s3_url) {
        ...
        list.push({ name: params.s3_name || "源3", url: cleanUrl(params.s3_url) });
    ...
    if (list.length === 0) {
        ...
        list.push({ name: "官方源", url: "https://api.dandanplay.net" });
...
async function getCommentsById(params) {
    ...
    try {
        ...
        if (data.comments && data.comments.segmentList) {
            ...
            await Widget.storage.set(commentIdKey, commentId); 
    ...
    if (segmentList && Array.isArray(segmentList)) {
        ...
        if (segment) {
            ...
            try {
                ...
                if (data.barrage\_list) comments = data.barrage\_list;
    ...
    return comments.map(c => {
        ...
        if (content) {
            ...
            if (convertMode === "t2s") {
                ...
                content = simplized(content); // 繁 -> 简

            } else if (convertMode === "s2t") {

                content = traditionalized(content); // 简 -> 繁
